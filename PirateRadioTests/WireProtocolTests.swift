...
struct ServerToClientTests {
    ...
    private func decode(_ json: String) -> SyncMessage? {
        ...
        return WebSocketTransport.translate(serverMsg, rawData: data)
    }
    ...
    func memberJoined() {
        ...
        let msg = decode(json)
        #expect(msg != nil)
        if case .memberJoined(let userID, let displayName) = msg?.type {
    ...
    @Test("queueUpdate decodes tracks from queue objects")
    func queueUpdate() {
        let json = """
        {"type":"queueUpdate","data":{"queue":[{"id":"t1","name":"Song 1","artist":"A","albumName":"Al","durationMs":180000},{"id":"t2","name":"Song 2","artist":"B","albumName":"Al","durationMs":200000}]},"epoch":1,"seq":10,"timestamp":1700000008000}
        """
        let msg = decode(json)
        #expect(msg != nil)
        if case .queueUpdate(let tracks) = msg?.type {
    ...
    func stateSync() {
        let json = """
        {"type":"stateSync","data":{"id":"session-1","joinCode":"1073","creatorId":"dj-1","djUserId":"dj-1","members":[{"userId":"dj-1","displayName":"DJ"}],"epoch":2,"sequence":15,"currentTrack":{"id":"track-abc","name":"Song","artist":"Art","albumName":"Al","durationMs":180000},"isPlaying":true,"positionMs":45000,"positionTimestamp":1700000000000,"queue":[{"id":"q1","name":"Q1","artist":"A","albumName":"Al","durationMs":180000},{"id":"q2","name":"Q2","artist":"B","albumName":"Al","durationMs":200000}]}}
        """
        let msg = decode(json)
        #expect(msg != nil)
...
struct ClientToServerTests {
    ...
    private func encode(_ message: SyncMessage) -> [String: Any] {
        WebSocketTransport.encodeForServer(message)
    }
